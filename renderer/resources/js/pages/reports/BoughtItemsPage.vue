    ...
    <div>
      ...
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
          ...
          <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('bought_items') || 'Bought Items Report' }}</h1>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('total_items_bought') || 'Total Items Bought' }}</p>
          <p class="text-2xl font-bold text-[#D4A843] mt-1">{{ totalQtyBought }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
          <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('total_cost') || 'Total Cost' }}</p>
          <p class="text-2xl font-bold text-red-600 dark:text-red-400 mt-1">{{ formatCurrency(totalCost) }}</p>
        </div>
      </div>
      ...
      <AppDataTable
        ...
        :data="boughtItems"
        :loading="loading"
        :empty-message="t('no_data') || 'No purchased items found'"
      >
        <template #cell-total_cost="{ item }">
          <span class="font-semibold text-gray-900 dark:text-white">{{ formatCurrency(item.total_cost) }}</span>
        </template>
...
const columns = [
  ...
  { key: 'qty_bought', label: t('qty_bought') || 'Qty Bought', sortable: true },
  { key: 'total_cost', label: t('total_cost') || 'Total Cost', sortable: true },
...
const boughtItems = computed(() => {
  ...
  invoices.value.forEach((inv) => {
    ...
    items.forEach((item) => {
      ...
      if (!productMap[name]) {
        productMap[name] = { product_name: name, qty_bought: 0, total_cost: 0 };
      }
      productMap[name].qty_bought += qty;
      productMap[name].total_cost += total;
    });
  });

  return Object.values(productMap).map((p) => ({
    ...p,
    avg_unit_price: p.qty_bought > 0 ? p.total_cost / p.qty_bought : 0,
  })).sort((a, b) => b.total_cost - a.total_cost);
});

const totalQtyBought = computed(() =>
  boughtItems.value.reduce((sum, i) => sum + i.qty_bought, 0)
);

const totalCost = computed(() =>
  boughtItems.value.reduce((sum, i) => sum + i.total_cost, 0)
);
...
async function fetchInvoices() {
  ...
  error.value = '';
  try {
    const res = await api.get('/invoices', { params: { type: 'purchase' } });
    invoices.value = res.data.data || res.data;
  } catch (err) {
    error.value = err.response?.data?.message || 'Failed to load purchase data.';
  } finally {
    loading.value = false;
  }
}

  ...
  <AppLayout>
    <div>
      ...
      <div class="flex items-center justify-between mb-6">
        <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ t('suppliers') }}</h1>
      ...
      >
        <template #empty>
          <div class="text-center py-16">
            ...
            <h3 class="text-lg font-medium text-gray-500 dark:text-gray-400">{{ t('no_suppliers_yet') || 'No suppliers yet' }}</h3>
            <p class="text-sm text-gray-400 dark:text-gray-500 mt-1">{{ t('add_first_supplier') || 'Add your first supplier to get started' }}</p>
            <AppButton variant="primary" class="mt-4" @click="openModal()">+ {{ t('add') }} {{ t('supplier') }}</AppButton>
          </div>
        </template>
        <template #cell-actions="{ item }">
          <div class="flex items-center gap-2">
            <AppButton size="sm" variant="ghost" @click="$router.push('/suppliers/' + item.uuid + '/statement')">
              {{ t('statement') }}
            </AppButton>
      ...
      <!-- Add/Edit Modal -->
      <AppModal :show="showFormModal" :title="editingItem ? t('edit') + ' ' + t('supplier') : t('add') + ' ' + t('supplier')" size="md" @close="showFormModal = false">
        <div class="space-y-4">
          <AppInput
            v-model="form.name"
            :label="t('name')"
      ...
      <AppModal :show="showDeleteModal" :title="t('confirm_delete')" size="sm" @close="showDeleteModal = false">
        <p class="text-gray-700 dark:text-gray-300">{{ t('confirm_delete_supplier') }}</p>
...
async function confirmDelete(item) {
  ...
  try {
    const res = await api.get('/invoices', { params: { supplier_uuid: item.uuid, per_page: 1 } });
...
async function submitForm() {
  ...
  try {
    if (editingItem.value) {
      await api.put('/suppliers/' + editingItem.value.uuid, form.value);
    } else {
...
async function deleteItem() {
  ...
  deleting.value = true;
  try {
    await api.delete('/suppliers/' + deleteTarget.value.uuid);
    items.value = items.value.filter((s) => s.uuid !== deleteTarget.value.uuid);

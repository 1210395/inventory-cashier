    ...
    <div>
      ...
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center gap-3">
          <AppButton variant="ghost" @click="$router.push('/customers')">
      ...
      <div v-if="!loading && statement">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
          <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
            <p class="text-sm text-gray-500 dark:text-gray-400">{{ t('customer') }}</p>
            <p class="text-lg font-bold text-gray-900 dark:text-white mt-1">{{ statement.customer?.name || '-' }}</p>
            <p v-if="statement.customer?.phone" class="text-xs text-gray-400 mt-1">{{ statement.customer.phone }}</p>
          </div>
          <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4">
...
async function fetchStatement() {
  ...
  try {
    ...
    const res = await api.get('/customers/' + route.params.uuid + '/statement', { params });
